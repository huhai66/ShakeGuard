...
一个**免 Root、免 adb、免 Shizuku** 的安卓广告自动跳过工具：开屏广告、摇一摇跳转、弹窗里的「跳过 / 关闭」按钮，由无障碍服务自动帮你点掉。单 APK 侧载即可使用。
...
| 自动返回 | 检测到被监控应用短时间内跳转到广告落地应用时，立即按返回拉回（防「摇一摇」跳走） |
...
- 若某个被监控应用在极短时间内跳到了广告落地应用（淘宝、京东、拼多多、抖音等），立即按返回键拉回原应用。
...
1. 安装 APK，打开应用。
2. 点击红色卡片上的按钮，在系统设置中开启本应用的无障碍服务。
3. 打开「自动跳过广告」总开关。
4. 在应用列表里勾选需要处理的应用（或点「全部勾选」）。
5. 按需开启「自动返回跳转」和「图像识别（网页广告）」。
...
│   └── SkipSettings.kt         # 全局设置（总开关/自动返回/图像识别/应用列表/跳过计数）
├── receiver/
│   └── BootReceiver.kt         # 开机自启
├── service/
│   ├── ShakeGuardAccessibilityService.kt  # 核心：识别并点击跳过/关闭按钮、自动返回
